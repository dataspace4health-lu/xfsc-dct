...
export class AgreementValidationService {
    constructor(private readonly trustService: AbstractTrustServiceAdapter, private readonly federatedCatalog: AbstractFederatedCatalogAdapter) { }
    ...
    async assertParticipant(dataAsset: DataAsset, type: ParticipantType) {
        ...
        if (isRevoked) {
            throw new HttpException(`Unavailable for legal reasons, Gaia-X Participant status has
            been revoked`, 451)
        }
    }
