...
export type MenuItemClickHandler = (event: { originalEvent: React.MouseEvent; item: any }) => void;

export const getMenuItemKey = (item: MenuItem) => item.key ?? item.label ?? item.to;
...
const AppSubmenu: React.FC<AppSubmenuProps> = (props) => {
    ...
    const onMenuItemClick = (event: React.MouseEvent, item: MenuItem, index: number) => {
        if (item.disabled) {
        ...
        if (item.to) {
            return (
                <NavLink
                    ...
                    role="menuitem"
                    className={({ isActive }) =>
                        cx('p-ripple', { 'router-link-active router-link-exact-active': isActive })
                    }
                    to={item.to}
