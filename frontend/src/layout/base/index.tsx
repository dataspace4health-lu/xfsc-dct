...
export const BaseLayout: React.FC<BaseLayoutProps> = ({ onSignOut }) => {
    const [layoutMode, setLayoutMode] = React.useState<'static' | 'overlay'>('static');
    const [layoutColorMode, setLayoutColorMode] = React.useState<'light' | 'dark'>('light');
    const [inputStyle, setInputStyle] = React.useState<'outlined' | 'filled'>('outlined');
    ...
    const [mobileTopbarMenuActive, setMobileTopbarMenuActive] = React.useState(false);
    ...
    const menu: MenuItem[] = [
        {
            items: [
                ...
                { label: 'Transaction Logs', icon: 'pi pi-fw pi-home', to: '/logs' },
